...
const UserProfile = (props) => {
  ...
  const pageUserID = props.match.params.id
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_ENDPOINT}/api/v1/user/${pageUserID}`)
      .then((res) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_ENDPOINT}/api/v1/post/user/${pageUserID}`
      )
      .then((res) => {
  ...
  return (
    <section className="userprofile__container">
      {error ? (
        <h1 className="userprofile__error">No User Found</h1>
      ) : (
        <React.Fragment>
